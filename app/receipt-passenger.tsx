...
export default function PassengerReceiptScreen() {
    ...
    async function loadReceiptData() {
      ...
      try {
        ...
        if (details) {
          ...
          // Prefer real passengerType from saved Trip; fall back to param or "Regular"
  ...
  const handleShareReceipt = async () => {
    try {
      ...
      if (isAvailable) {
        ...
        await Share.share({ url: tempFilePath, title: "QRTransit Receipt", message: "Here's my receipt from QRTransit!" })
      }
    } catch (error) {
  ...
  const handleGoBack = () => {
    if (params.from && params.to) router.replace("/")
    else router.replace("/")
  }
  ...
  return (
    <ScrollView style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>Passenger Receipt</Text>
      ...
      <ViewShot ref={viewShotRef} style={styles.receiptContainer} options={{ format: "png", quality: 1 }}>
        ...
        {/* ✅ Always render Bus Type with fallback */}
        <View style={styles.infoRow}>
